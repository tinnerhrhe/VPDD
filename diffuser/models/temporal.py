...
class Tasksmaze(nn.Module):
    ...
    def forward(self, x, time, cond, value, context_mask, x_condition=None, force=False, return_cond=False, flag=False, attention_mask=None):
        ...
        cond = cond.long()
        #x_condition = einops.rearrange(x_condition, 'i j k -> i (j k)')
        #prompt_data = get_prompt_maze2d(self.prompt_trajectories, cond, num_episodes=1, num_steps=50, is_meta=False)
...
class TasksmetaAug(nn.Module):
    ...
    def __init__(
        ...
        task_list = ['bin-picking-v2', 'box-close-v2', 'door-lock-v2', 'door-unlock-v2', 'hand-insert-v2']
        self._prompt = [np.load(f"./metaworld_prompts/{ind}_prompt.npy", allow_pickle=True) for ind in task_list]
        #self.predict_return = torch.nn.Linear(hidden_size, 1)
    ...
    def forward(self, x, time, cond, x_condition=None, force=False, return_cond=False, flag=False, attention_mask=None):
        ...
        cond = cond.long()
        if cond[0] < 0:
            prompt_data = torch.tensor([get_prompt(self._prompt[-ind-1], 1, 20, True) for ind in cond])
        else:
...
class TasksOnehot(nn.Module):
    ...
    def __init__(
        ...
        self.onehot_embed = nn.Sequential(
            nn.Linear(self.num_tasks, hidden_size*2),  # TODO
            nn.Mish(),
            nn.Linear(hidden_size * 2, 4 * hidden_size),
            nn.Mish(),
            nn.Linear(4 * hidden_size, hidden_size),
        ...
        self.embed_ln = nn.LayerNorm(hidden_size)
        self.embed_act = nn.Linear(self.action_dim, hidden_size)
        self.position_emb = nn.Parameter(torch.zeros(1, 5+horizon, hidden_size))
    ...
    def forward(self, x, time, cond, value, context_mask, x_condition=None, force=False, return_cond=False, flag=False, attention_mask=None):
        ...
        cond = cond.long()
        #x_condition = einops.rearrange(x_condition, 'i j k -> i (j k)')
        #prompt_data = get_prompt_batchs(self.prompt_trajectories, cond, num_episodes=1, num_steps=20, is_meta=False)
        #prompt_data = prompt_data.to(device=x.device, dtype=torch.float32).reshape(cond.shape[0], -1, self.state_dim+self.action_dim)#TODO
        #prompt_embeddings = self.prompt_embed(prompt_data)
        onehot = nn.functional.one_hot(-cond, num_classes=self.num_tasks).type(torch.float)
        if cond[0] < 0:
            onehot = torch.zeros_like(onehot)
        onehot_embeddings = self.onehot_embed(onehot).unsqueeze(1)
        obs_embeddings = self.embed_obs(x_condition)
        t = self.time_mlp(time).unsqueeze(1)
        if not force:
            mask = self.mask_dist.sample(sample_shape=(cond.shape[0], 1)).to(x.device)
        ...
        batch_size, seq_length = x.shape[0], x.shape[1]
        value = value.view(-1, 1)
        value = self.return_mlp(value * mask)
        cond_return = value.unsqueeze(1)
        #cond_return = mask * value
        #cond_return = cond_return.unsqueeze(1)
        act_embeddings = self.embed_act(x)
        addition_length = 1 + 1 + 1 + 2  #TODO
        addition_attention_mask = torch.ones((batch_size, addition_length), dtype=torch.long, device=x.device)
        if attention_mask is None:
            # attention mask for GPT: 1 if can be attended to, 0 if not
            attention_mask = torch.ones((batch_size, seq_length), dtype=torch.long, device=x.device)
            '''set attention mask'''
        stacked_inputs = torch.cat((t, cond_return, onehot_embeddings, obs_embeddings, act_embeddings), dim=1)
        stacked_attention_mask = torch.cat(
            (addition_attention_mask, attention_mask), dim=1)
...
class Taskstext(nn.Module):
    ...
    def __init__(
        ...
        self.text_embed = nn.Sequential(
            nn.Linear(768, hidden_size*2),  # TODO
            nn.Mish(),
            nn.Linear(hidden_size * 2, 4 * hidden_size),
            nn.Mish(),
            nn.Linear(4 * hidden_size, hidden_size),
        ...
        self.predict_act = torch.nn.Linear(hidden_size, self.action_dim)
        #self.predict_return = torch.nn.Linear(hidden_size, 1)
        with open('./embeddings_mt50.pkl', "rb") as fIn:
            stored_data = pickle.load(fIn)
            self.stored_embeddings = stored_data['embeddings']
    ...
    def forward(self, x, time, cond, value, context_mask, x_condition=None, force=False, return_cond=False, flag=False, attention_mask=None):
        ...
        #cond_embedding = stored_embeddings[cond.cpu()]
        cond_embedding = self.stored_embeddings[[self.stored_sentences.index(i) for i in np.array(self.task_list)[cond.cpu()]]]
        text_embeddings = self.text_embed(torch.as_tensor(cond_embedding, dtype=torch.float32, device=x.device)).unsqueeze(1)
        obs_embeddings = self.embed_obs(x_condition)
        t = self.time_mlp(time).unsqueeze(1)
        if not force:
            mask = self.mask_dist.sample(sample_shape=(cond.shape[0], 1)).to(x.device)
        ...
        if attention_mask is None:
            ...
            attention_mask = torch.ones((batch_size, seq_length), dtype=torch.long, device=x.device)
            '''set attention mask'''
        stacked_inputs = torch.cat((t, cond_return, text_embeddings, obs_embeddings, act_embeddings), dim=1)
        stacked_attention_mask = torch.cat(
            (addition_attention_mask, attention_mask), dim=1)
