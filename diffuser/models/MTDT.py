...
class DecisionTransformer(nn.Module):
    ...
    def __init__(
        ...
        super().__init__()
        self.state_dim = state_dim + num_tasks
    ...
    def forward(self, states, actions, rewards, returns_to_go, timesteps, attention_mask=None, task=None):
        ...
        timesteps = timesteps.to(dtype=torch.long)# if timesteps.dtype is not torch.long else timesteps
    ...
    def get_action(self, states, actions, rewards, returns_to_go, timesteps, batch_size=1, **kwargs):
        ...
        if self.max_length is not None:
            ...
            attention_mask = attention_mask.to(dtype=torch.long, device=states.device).reshape(1, -1).repeat(batch_size, 1)
            states = torch.cat(
...
class PromptDT(nn.Module):
    ...
    def forward(self, states, actions, rewards, returns_to_go, timesteps, attention_mask=None, prompt=None):
        ...
        if prompt is not None:
            prompt_states, prompt_actions, prompt_rewards, prompt_returns_to_go, prompt_timesteps, prompt_attention_mask \
                = prompt.observations, prompt.actions, None, prompt.rtg, prompt.timestep, prompt.mask
            prompt_timesteps = prompt_timesteps.to(dtype=torch.long)
...
class PromptDTMaze(nn.Module):
    ...
    def __init__(
        ...
        #self.prompt_embed_state = torch.nn.Linear(self.state_dim, hidden_size)
        #self.prompt_embed_action = torch.nn.Linear(self.act_dim, hidden_size)
        self.prompt_embed = torch.nn.Linear(35, hidden_size)
        self.embed_ln = nn.LayerNorm(hidden_size)
    ...
    def forward(self, states, actions, rewards, returns_to_go, timesteps, attention_mask=None, prompt=None):
        ...
        # process prompt the same as d-t
        if prompt is not None:
            prompt_embeddings = self.prompt_embed(prompt).unsqueeze(1)
            prompt_stacked_inputs = prompt_embeddings
            prompt_stacked_attention_mask = torch.ones((prompt_embeddings.shape[0], prompt_embeddings.shape[1]),
                                                       device=stacked_inputs.device, dtype=torch.long)
            stacked_inputs = torch.cat((prompt_stacked_inputs, stacked_inputs), dim=1)
            stacked_attention_mask = torch.cat((prompt_stacked_attention_mask, stacked_attention_mask), dim=1)
        # we feed in the input embeddings (not word indices as in NLP) to the model
        transformer_outputs = self.transformer(
