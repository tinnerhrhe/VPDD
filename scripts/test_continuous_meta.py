...
def evaluate(task):
    ...
    for env_id, env_ in enumerate([task]):
        ...
        env_list = [create_env_and_policy(env_, seed=idx) for idx in range(num_evals)]
        ...
        for step in tqdm(range(0, max_episode_length),desc="Episode timestep ", total=max_episode_length):
            ...
            #import pdb;pdb.set_trace()
            samples = diffusion.conditional_sample(x_condition, task=lang_feats.unsqueeze(0).repeat(num_evals,1,1).to(dtype=torch.float32), x_condition=x_condition, horizon=4, guidance=1.2)
            ...
            for i in range(len(env_list)):
                ...
                condition[i, 0] = torch.from_numpy(np.rot90(env_list[i].render(), 2)[110:370,110:370].copy()).to(args.device)
