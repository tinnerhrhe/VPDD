...
# ----------------------------------- setup -----------------------------------#
# -----------------------------------------------------------------------------#
ASSET_DIR = "/home/rg5/hehaoran/data"
DTYPE = torch.float
DEVICE = 'cuda'
def cycle(dl):
    while True:
...
def create_env_and_policy(env_name, seed=None):
    ...
    env_cls = ALL_V2_ENVIRONMENTS_GOAL_HIDDEN[f'{env_name}-goal-hidden']
    env_kwargs = dict(
      camera_name='corner2',
      # Needed for rendering
      get_image_obs=True,
      image_obs_size=(480,480),
      )
    factor_kwargs={
        #   'floor_texture': dict(
        #       seed=args.seed,
...
def evaluate(task):
    ...
    for env_id, env_ in enumerate([task]):
        ...
        for each in env_list:
            each.render()
            obs_list.append(each.reset())
        #obs_list = [env.reset()[0][None] for env in env_list]
        condition = torch.zeros([num_evals, 1, 260, 260, 3], device=args.device)
        #import pdb;pdb.set_trace()
        for i, env in enumerate(env_list):
            condition[i,-1] = torch.from_numpy(obs_list[i]['image'].transpose(1,2,0)[:,::-1][110:370,110:370].copy()).to(args.device)
        ...
        for step in tqdm(range(0, max_episode_length),desc="Episode timestep ", total=max_episode_length):
            ...
            #import pdb;pdb.set_trace()
            act = diffusion.sample_mask(num_evals, task=lang_feats.unsqueeze(0).repeat(num_evals,1,1).to(dtype=torch.float32), x_condition=x_condition)
            
            trans = action_tokenizer.reconstruct(act[:,0,:3])
            grippers = gripper_list[(act[:,0,-1:]).cpu().numpy()]
            action = np.concatenate([trans, grippers], axis=-1)
            for i in range(len(env_list)):
                obs, reward, done, info = env_list[i].step(action[i])
                condition[i] = torch.cat([condition[i,1:], torch.from_numpy(obs_list[i]['image'].transpose(1,2,0)[:,::-1][110:370,110:370].copy()).to(args.device).unsqueeze(0)],dim=0)
        ...
        #os.makedirs(f'./eval_r3m_{args.seed}.txt', exist_ok=True)
        with open(os.path.join(f'./eval_r3m_{args.seed}.txt'), 'a+') as f:
...
"""
ps -ef|grep Xvfb
# Xvfb :6-screen 0 1336x768x24 2>/dev/null &
# x11vnc -listen 0.0.0.0 -rfbport 5901 -noipv6 -passwd 123456 -display :6
